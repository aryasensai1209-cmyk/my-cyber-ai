...
class EnterpriseLogicEngine:
    def __init__(self):
        self.vulnerability_map = {
            '1. Injection & Input': [
                {'name': 'Advanced SQLi', 'pattern': r'(SELECT|INSERT|UPDATE|DELETE|UNION|DROP|EXEC|CAST|CONVERT).*?([\\s\\+\\$\\%{]).*?[\\x27\\x22]', 'explanation': 'Complex SQL injection vectors via string concatenation.', 'fix': 'Enforce parameterized queries.', 'severity': 'CRITICAL'},
                {'name': 'Shell Command Injection', 'pattern': r'(os\\.system|subprocess|exec|eval|system|popen|shutil|spawn)\\(.*?([\\+\\%]|f[\\"\\\\]).+?\\)', 'explanation': 'Shell-level execution vulnerabilities.', 'fix': 'Avoid shell=True; use list-based arguments.', 'severity': 'CRITICAL'}
            ],
            '2. Web App Logic': [
...
tab1, tab2, tab3, tab4 = st.tabs(['🔍 Scanner', '📊 Analytics', '📋 Precision Remediation (JSON)', '🧪 Signature Debugger'])

with tab1:
    code_input = st.text_area('Input Code:', height=300, placeholder='Paste code for 15-vector scan...')
    if st.button('INITIATE DEEP INSPECTION'):
        if code_input:
            results, latency = st.session_state.engine.scan(code_input)
            st.session_state.results = results
            st.session_state.latency = latency
            if results: st.error(f'Threats Found: {len(results)}')
...
with tab2:
    if 'latency' in st.session_state:
        ...
        m2.metric('Throughput', f"{1/(st.session_state.latency + 1e-9):,.0f} ops/sec")

with tab3:
    st.subheader('Precision Analysis Report')
    if 'results' in st.session_state and st.session_state.results:
        st.json(st.session_state.results)
    else: st.info('Run a scan to generate a professional JSON report.')

with tab4:
    st.subheader('🧪 Precision Debugger')
    st.info('Isolate specific fragments triggering regex matches.')
    test_line = st.text_input('Code Fragment:', placeholder='e.g. query = "SELECT * FROM..." + id')
    test_regex = st.text_input('Regex Signature:', value=r'(SELECT|INSERT|UPDATE|DELETE).*?([\\s\\+\\$\\%{]).*?[\\x27\\x22]')
    if st.button('VALIDATE FRAGMENT'):
